...
namespace CloudBackup {
  ...
  class CompressUtil
  {
    public:
      ...
      static bool compress(const std::string &src, const std::string &dst) {
		...
		int wlen = 0;
		while(wlen < buf.length()){
		  int ret = gzwrite(file, buf.c_str(), buf.length()-wlen);
		  if (ret == 0) {
  ...
  class FileDataManager 
  {
    ...
    bool getAllList(std::vector<std::string> &fileList) {
      ...
      m_mutex.lock();
      for (auto &it : m_map) {
        fileList.push_back(it.first);	// 获取得失源文件名称
      }
      m_mutex.unlock();
      return true;
    }
    ...
    private:
    std::string m_filename;	// 持久化数据存储文件名称
    std::unordered_map<std::string, FileData> m_map;	// 数据管理容器
    std::mutex m_mutex;
  };
        ...
        while (true) {
          ...
          for (std::vector<int>::size_type i = 0; i < fileList.size(); ++i) {
            ...
            if (!m_fdm.isCompressedFile(fileList[i]) && MyUtil::isNonHotFile(fileList[i], m_s_IntervalTime)) {
              CompressUtil::compress(fileList[i], fileList[i] + ".gz");	// 不是热点文件，压缩备份
              unlink(fileList[i].c_str());	// 删除一个名称从文件中（删除文件目录项）
              m_fdm.changeData(fileList[i]);
            }
          }
          MyUtil::MySleep(m_s_IntervalTime);	// 休眠一会
        }
      }
    private:
      FileDataManager &m_fdm;
      static const time_t m_s_IntervalTime = 30;	// 间隔时间
  };
  ...
  class HttpServerModule
  {
    public:
      ...
      void start(const std::string &host = "0.0.0.0", int port = 9000) {
        ...
        m_srv.listen(host.c_str(), port);	// 搭建tcp服务器，进行http数据接收处理
      }
    private:
      ...
      static void _fileUpload(const httplib::Request &req, httplib::Response &res) {
		...
		// 向文件写入数据（文件不存在会创建）｜｜添加文件信息到数据管理模块
      ...
      static void _fileDownload(const httplib::Request &req, httplib::Response &res) {
        ...
        if (fdManager.isCompressedFile(filepath)) {
          ...
          CompressUtil::decompress(filepath + ".gz", filepath);	// 将压缩包解压
          unlink((filepath + ".gz").c_str());	// 删除压缩包
        }
		// 从文件中读取数据，响应给客户端
        MyUtil::readFile(filepath, res.body);
        res.status = 200;
        res.set_header("Content-Type", "application/octet-stream");	// 二进制流下载
